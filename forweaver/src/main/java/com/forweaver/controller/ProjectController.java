...
public class ProjectController {
	...
	public void getProjectZip(@PathVariable("projectName") String projectName,
			@PathVariable("creatorName") String creatorName, @PathVariable("commitName") String commitName,
	...
	public String projectsWithPage(@PathVariable("page") String page, @PathVariable("sort") String sort, Model model) {
		int pageNum = WebUtil.getPageNumber(page);
		int size = WebUtil.getPageSize(page, 0);

		Weaver currentWeaver = weaverService.getCurrentWeaver();
		model.addAttribute("projects", projectService.getProjects(currentWeaver, null, "", sort, pageNum, size));
	...
	public String projectsWithTags(@PathVariable("tagNames") String tagNames, @PathVariable("page") String page,
		...
		Weaver currentWeaver = weaverService.getCurrentWeaver();
		model.addAttribute("projects", projectService.getProjects(currentWeaver, tagList, null, sort, pageNum, size));
	...
	public String tagsWithSearch(@PathVariable("tagNames") String tagNames, @PathVariable("page") String page,
			@PathVariable("search") String search, @PathVariable("sort") String sort, Model model) {
		List<String> tagList = tagService.stringToTagList(tagNames);
		int pageNum = WebUtil.getPageNumber(page);
		int size = WebUtil.getPageSize(page, 0);
	...
	public String add(@RequestParam Map<String, String> params, Model model) {
		...
		if (!Pattern.matches("^[a-z]{1}[a-z0-9_]{4,14}$", params.get("name")) || params.get("name").length() < 5
				|| params.get("description").length() < 5 || params.get("description").length() > 50
		...
		// 프로젝트 설정//
		Project project = new Project(params.get("name"), categoryInt, params.get("description"), currentWeaver,
				tagList);
	...
	public String delete(Model model, @PathVariable("creatorName") String creatorName,
		...
		if (projectService.delete(currentWeaver, project))
			for (Post post : postService.getPosts(tags, null, null, "", 1, Integer.MAX_VALUE)) // 프로젝트에
																								// 쓴
																								// 글
																								// 모두
																								// 삭제
	...
	public void data(HttpServletRequest request, @PathVariable("projectName") String projectName,
		...
		if (svnFileInfo == null) {
			...
			byte[] imgData = svnFileInfo.getData();

			res.reset();
			res.setContentType("application/octet-stream");
			String filename = new String(svnFileInfo.getName().getBytes("UTF-8"), "8859_1");
			res.setHeader("Content-Disposition", "attachment; filename = " + filename);
			res.setContentType(WebUtil.getFileExtension(svnFileInfo.getName()));
	...
	@RequestMapping(value = { "/{creatorName}/{projectName}", "/{creatorName}/{projectName}/browser" })
	public String browser(@PathVariable("projectName") String projectName,
		...
		return "redirect:/project/" + creatorName + "/" + projectName + "/browser/commit:" + gitBranchList.get(0)
				+ "/filepath:/";
	}
	...
	public String fileBrowser(HttpServletRequest request, @PathVariable("projectName") String projectName,
			@PathVariable("creatorName") String creatorName, @PathVariable("commit") String commit, Model model)
		...
		if (svnFileInfo == null || svnFileInfo.isDirectory()) { // 만약에 주소의 파일이
																// 디렉토리라면
			// 디렉터리이기에 파일내용은 필요없는 VCSimpleFileInfo를 생성//
			commit = "-1";

			List<VCSimpleFileInfo> svnFileInfoList = svnService.getVCSimpleFileInfoList(creatorName, projectName,
					commit, filePath);
			 ...
			 * gitService.getBranchList(creatorName, projectName);
			...
			if (svnFileInfo.getContent() != null)
				model.addAttribute("fileContent", new String(
						svnFileInfo.getContent().getBytes(Charset.forName("EUC-KR")), Charset.forName("CP949")));
	...
	public String blame(HttpServletRequest request, @PathVariable("projectName") String projectName,
		...
		if (svnFileInfo == null || svnFileInfo.isDirectory()) // 디렉토리의 경우 blame
																// 기능을 이용할 수 없어
																// 프로젝트 메인으로
																// 돌려보냄.
	...
	public String community(@PathVariable("projectName") String projectName, @PathVariable("sort") String sort,
			@PathVariable("creatorName") String creatorName, @PathVariable("page") String page, Model model) {
		int pageNum = WebUtil.getPageNumber(page);
		int size = WebUtil.getPageSize(page, 0);
		...
		model.addAttribute("project", project);
		model.addAttribute("posts", postService.getPosts(tags, null, null, sort, pageNum, size));
		model.addAttribute("postCount", postService.countPosts(tags, null, null, sort));
		model.addAttribute("pageIndex", pageNum);
		model.addAttribute("pageUrl",
				"/project/" + creatorName + "/" + projectName + "/community/sort:" + sort + "/page:");
	...
	public String tags(@PathVariable("projectName") String projectName, @PathVariable("creatorName") String creatorName,
			@PathVariable("tagNames") String tagNames, @PathVariable("sort") String sort,
		...
		model.addAttribute("project", project);
		model.addAttribute("posts", postService.getPosts(tagList, null, null, sort, pageNum, size));
		model.addAttribute("postCount", postService.countPosts(tagList, null, null, sort));
	...
	public String addPost(@PathVariable("projectName") String projectName,
			@PathVariable("creatorName") String creatorName, HttpServletRequest request, Model model) {
		...
		if (title.length() < 5 || title.length() > 200 || (content.length() > 0 && content.length() < 5)) { // 검증함
			model.addAttribute("say", "잘못 입력하셨습니다!!!");
	...
	public String commitLog(@PathVariable("projectName") String projectName,
		...
		// List<String> gitBranchList = gitService.getBranchList(creatorName,
		// projectName);

		// model.addAttribute("gitBranchList", gitBranchList.subList(1,
		// gitBranchList.size()));
		// model.addAttribute("selectBranch",gitBranchList.get(0));

		model.addAttribute("project", project);
		model.addAttribute("pageIndex", 1);
		model.addAttribute("gitCommitListCount", svnService.getCommitListCount(creatorName, projectName, "-1"));
		model.addAttribute("gitCommitList", svnService.getVCCommitLogList(creatorName, projectName, "-1", 1, 15));
	 ...
	 * 
	 * @ResponseBody public String rss(@PathVariable("projectName") String
	 * projectName,
	 * 
	 * @PathVariable("creatorName") String creatorName,Model model) {
	 * List<String> gitBranchList = gitService.getBranchList(creatorName,
	 ...
	 * 
	 * rss +="<title>project:"+creatorName+"/"+projectName+"</title>"; rss
	 * +="<link>http://forweaver.com/project/"+creatorName+"/"+projectName+
	 * "</link>"; rss
	 * +="<description>project:"+creatorName+"/"+projectName+"</description>";
	 * 
	 * for(GitSimpleCommitLog
	 * commitLog:gitService.getGitCommitLogList(creatorName,
	 * projectName,gitBranchList.get(0),1,10)){ rss +="<item>"; rss
	 * +="<author>"+commitLog.getCommiterName()+" ("+commitLog.getCommiterEmail(
	 * )+")</author>"; rss +="<title>"+commitLog.getShortMassage()+"</title>";
	 * rss +="<link>http://forweaver.com/project/"+creatorName+"/"+projectName+
	 * "</link>"; rss
	 * +="<description>"+commitLog.getShortMassage()+"</description>"; rss
	 * +="<pubDate>"+commitLog.getCommitDate()+"</pubDate>"; rss
	 * +="<image>http://forweaver.com/"+commitLog.getImgSrc()+"</image>"; rss
	 * +="</item>"; } return rss+"</channel></rss>"; }
	 * 
	 */
	...
	public String commitLog(@PathVariable("projectName") String projectName,
		...
		model.addAttribute("project", project);
		model.addAttribute("pageIndex", 1);
		model.addAttribute("gitCommitListCount", gitService.getCommitListCount(creatorName, projectName, commit));
		model.addAttribute("gitCommitList", gitService.getGitCommitLogList(creatorName, projectName, commit, 1, 15));
		return "/project/commitLog";
	}

	@RequestMapping("/{creatorName}/{projectName}/edit")
	public String edit(@PathVariable("projectName") String projectName, @PathVariable("creatorName") String creatorName,
			Model model) {
		Project project = projectService.get(creatorName + "/" + projectName);
		Weaver currentWeaver = weaverService.getCurrentWeaver();
	...
	public String edit(@PathVariable("projectName") String projectName, @PathVariable("creatorName") String creatorName,
		...
		if (!project.getCreator().equals(currentWeaver) || params.get("description") == null
				|| !tagService.isPublicTags(tagList)) {
			model.addAttribute("say", "잘못 입력하셨습니다!!!");
	...
	public String commitLog(@PathVariable("projectName") String projectName,
			@PathVariable("creatorName") String creatorName, @PathVariable("commit") String commit,
			@PathVariable("page") String page, HttpServletRequest request, Model model) {
		Project project = projectService.get(creatorName + "/" + projectName);
		String uri = request.getRequestURI();
		commit = uri.substring(uri.indexOf("/commit:") + 8);
		commit = commit.substring(0, commit.indexOf("/"));
		 ...
		 * projectName);
		 * 
		 * gitBranchList.remove(commit); model.addAttribute("gitBranchList",
		 * gitBranchList); model.addAttribute("selectBranch",commit);
		 */
		model.addAttribute("project", project);
	...
	public String commitLogViewer(@PathVariable("projectName") String projectName,
		...
		// model.addAttribute("rePosts",
		// rePostService.get(project.getName()+"/"+gitCommitLog.getCommitLogID(),5,""));
	...
	@RequestMapping(value = "/{creatorName}/{projectName}/weaver/{weaverName}/delete") // 회원
																						// 삭제용
	public String deleteWeaver(@PathVariable("projectName") String projectName,
			@PathVariable("creatorName") String creatorName, @PathVariable("weaverName") String weaverName,
			Model model) {
		Project project = projectService.get(creatorName + "/" + projectName);
		Weaver currentWeaver = weaverService.getCurrentWeaver();
		...
		if (projectService.deleteWeaver(project, currentWeaver, deleteWeaver)) {
			...
			if (currentWeaver.equals(project.getCreator())) {// 관리자가 탈퇴시킬시에 메세지
				post = new Post(currentWeaver, deleteWeaver.getId() + "님을 탈퇴 처리하였습니다.", "",
						tagService.stringToTagList("@" + project.getName() + ",탈퇴"));// 프로젝트에
																						// 메세지
																						// 보냄
				postService.add(post, null);
				post = new Post(currentWeaver, deleteWeaver.getId() + "님이 프로젝트명:" + project.getName() + "에서 탈퇴 당하셨습니다.",
						"", tagService.stringToTagList("$" + deleteWeaver.getId()));// 프로젝트에
																					// 메세지
																					// 보냄
				postService.add(post, null);
				...
				post = new Post(currentWeaver, deleteWeaver.getId() + "님이 탈퇴하셨습니다.", "",
						tagService.stringToTagList("@" + project.getName() + ",탈퇴"));// 프로젝트에
																						// 메세지
																						// 보냄
				postService.add(post, null);
	...
	public String addWeaver(@PathVariable("projectName") String projectName,
		...
		if (!waitingWeaver.equals(proposer) && waitJoinService.isCreateWaitJoin(project, waitingWeaver, proposer)) {
			...
			String title = "프로젝트명:" + creatorName + "/" + projectName + "에 가입 초대를 </a><a href='/project/" + creatorName
					+ "/" + projectName + "/weaver/" + waitingWeaver.getId() + "/join-ok'>승락하시겠습니까?</a> "
					+ "아니면 <a href='/project/" + creatorName + "/" + projectName + "/weaver/" + waitingWeaver.getId()
					+ "/join-cancel'>거절하시겠습니까?</a><a>";

			Post post = new Post(projectCreator, title, "", tagService.stringToTagList("$" + waitingWeaver.getId()),
					true);
			waitJoinService.createWaitJoin(project.getName(), proposer.getId(), waitingWeaver.getId(),
					postService.add(post, null));
			model.addAttribute("url", "/project/" + creatorName + "/" + projectName + "/weaver/");
	...
	public String join(@PathVariable("projectName") String projectName, @PathVariable("creatorName") String creatorName,
			Model model) {
		Project project = projectService.get(creatorName + "/" + projectName);
		...
		if (waitJoinService.isCreateWaitJoin(project, waitingWeaver, waitingWeaver)) {
			String title = waitingWeaver.getId() + "님이 프로젝트명:" + creatorName + "/" + projectName
					+ "에 가입 신청을 </a><a href='/project/" + creatorName + "/" + projectName + "/weaver/"
					+ waitingWeaver.getId() + "/join-ok'>승락하시겠습니까?</a> " + "아니면 <a href='/project/" + creatorName + "/"
					+ projectName + "/weaver/" + waitingWeaver.getId() + "/join-cancel'>거절하시겠습니까?</a><a>";
			Post post = new Post(waitingWeaver, title, "", tagService.stringToTagList("$" + project.getCreatorName()),
					true);
			waitJoinService.createWaitJoin(project.getName(), waitingWeaver.getId(), waitingWeaver.getId(),
					postService.add(post, null));
			model.addAttribute("url", "/");
	...
	@RequestMapping("/{creatorName}/{projectName}/weaver/{weaver}/join-ok") // 프로젝트
																			// 가입
																			// 승인
	public String joinOK(@PathVariable("projectName") String projectName,
		...
		if (waitJoinService.isOkJoin(waitJoin, project.getCreatorName(), currentWeaver) // 요청자가
																						// 쪽지를
																						// 보내고
																						// 관리자가
																						// 승인을
			...
			Post post = new Post(
					waitingWeaver, "관리자 " + project.getCreatorName() + "님의 승인으로 프로젝트명:" + creatorName + "/"
							+ projectName + "에 가입이 승인되었습니다!",
					"", tagService.stringToTagList("@" + project.getName() + ",가입"), true); // @프로젝트명,가입
																							// 태그를
																							// 걸어줌
			...
			Post post = new Post(currentWeaver, // 가입자가 관리자에게 보내는 메세지
					currentWeaver.getId() + "님이 프로젝트명:" + creatorName + "/" + projectName + "를 가입 초대를 수락하셨습니다!", "",
					tagService.stringToTagList("@" + project.getName() + ",가입"), true); // @프로젝트명,가입
																						// 태그를
																						// 걸어줌
	...
	@RequestMapping("/{creatorName}/{projectName}/weaver/{weaver}/join-cancel") // 프로젝트
																				// 가입
																				// 승인
																				// 취소
	public String joinCancel(@PathVariable("projectName") String projectName,
		...
		if (project != null // 요청자가 쪽지를 보내고 관리자가 승인을 하는 경우
			...
			Post post = new Post(currentWeaver, // 관리자가 가입자에게 보내는 메세지
					"관리자 " + project.getCreatorName() + "님의 프로젝트명:" + creatorName + "/" + projectName
							+ "에 가입이 거절되었습니다.",
			...
			postService.delete(project.getCreator(), postService.get(waitJoin.getPostID()));
			Post post = new Post(currentWeaver, // 가입자가 관리자에게 보내는 메세지
					currentWeaver.getId() + "님이 프로젝트명:" + creatorName + "/" + projectName + "를 가입 초대를 거절하셨습니다.", "",
	...
	public String upload(@PathVariable("projectName") String projectName,
			@PathVariable("creatorName") String creatorName, @PathVariable("branchName") String branchName,
			@RequestParam("message") String message, @RequestParam("path") String path,
	...
	public String info(@PathVariable("projectName") String projectName, @PathVariable("creatorName") String creatorName,
			Model model) {
		Project project = projectService.get(creatorName + "/" + projectName);
		model.addAttribute("project", project);
	...
	public String chat(@PathVariable("projectName") String projectName, @PathVariable("creatorName") String creatorName,
			Model model) {
		Project project = projectService.get(creatorName + "/" + projectName);
		Weaver currentWeaver = weaverService.getCurrentWeaver();
		model.addAttribute("project", project);
	...
	/*
	 * @RequestMapping("/{creatorName}/{projectName}/fork") // 포크 public String
	 * fork(@PathVariable("projectName") String projectName,
	 * 
	 * @PathVariable("creatorName") String creatorName,Model model){ Project
	 * project = projectService.get(creatorName+"/"+projectName); Weaver
	 * currentWeaver = weaverService.getCurrentWeaver();
	 * 
	 * String newProjectName= projectService.fork(project, new
	 * Project(projectName, currentWeaver, project), currentWeaver);
	 * 
	 * if(newProjectName==null){ model.addAttribute("url",
	 * "/project/"+project.getName()); model.addAttribute("say", "포크하지 못했습니다!");
	 * return "/alert"; }else{ return "redirect:/project/"+newProjectName; } }
	 * /*
	 * 
	 * @RequestMapping("/{creatorName}/{projectName}/cherry-pick") // 채리픽 요청 목록
	 * public String cherryPickRequests(@PathVariable("projectName") String
	 * projectName,
	 * 
	 * @PathVariable("creatorName") String creatorName,Model model){ Project
	 * project = projectService.get(creatorName+"/"+projectName); List<String>
	 * gitBranchList = gitService.getBranchList(creatorName, projectName);
	 * 
	 * model.addAttribute("project", project); model.addAttribute("cherryPicks",
	 * cherryPickRequestService.get(project));
	 * model.addAttribute("gitBranchList", gitBranchList.subList(1,
	 * gitBranchList.size()));
	 * model.addAttribute("selectBranch",gitBranchList.get(0)); return
	 * "/project/cherryPick"; }
	 * 
	 * @RequestMapping(
	 * "/{creatorName}/{projectName}/cherry-pick/commit:{commit}/add") // 채리픽 요청
	 * 추가 public String addCherryPickRequest(@PathVariable("projectName") String
	 * projectName,
	 * 
	 * @PathVariable("creatorName") String creatorName,@PathVariable("commit")
	 * String commit, HttpServletRequest request){ Project project =
	 * projectService.get(creatorName+"/"+projectName); Weaver currentWeaver =
	 * weaverService.getCurrentWeaver(); String uri = request.getRequestURI();
	 * commit = uri.substring(uri.indexOf("/commit:")+8); commit =
	 * commit.substring(0, commit.indexOf("/"));
	 * 
	 * if(!project.isForkProject()) return
	 * "redirect:/project/"+creatorName+"/"+projectName+
	 * "/commitlog-viewer/commit:"+commit;
	 * 
	 * Project orginalProject =
	 * projectService.get(project.getOriginalProjectName());
	 * 
	 * if(!gitService.existCommit(orginalProject.getName().split("/")[0],
	 * orginalProject.getName().split("/")[1], commit) &&
	 * cherryPickRequestService.add(orginalProject, project, currentWeaver,
	 * commit)) return
	 * "redirect:/project/"+project.getOriginalProjectName()+"/cherry-pick";
	 * 
	 * return "redirect:/project/"+creatorName+"/"+projectName+
	 * "/commitlog-viewer/commit:"+commit; }
	 * 
	 * @RequestMapping(
	 * "/{creatorName}/{projectName}/cherry-pick/branch:{branch}/id:{id}/accept")
	 * // 채리픽 요청 수락 public String
	 * acceptCherryPickRequest(@PathVariable("projectName") String projectName,
	 * 
	 * @PathVariable("creatorName") String creatorName,@PathVariable("branch")
	 * String branch,@PathVariable("id") String id){ Weaver currentWeaver =
	 * weaverService.getCurrentWeaver(); CherryPickRequest cherryPickRequest =
	 * cherryPickRequestService.get(id);
	 * cherryPickRequestService.accept(cherryPickRequest, branch,
	 * currentWeaver); return
	 * "redirect:/project/"+creatorName+"/"+projectName+"/cherry-pick"; }
	 * 
	 * @RequestMapping(
	 * "/{creatorName}/{projectName}/cherry-pick/id:{id}/delete") // 채리픽 요청 삭제
	 * public String deleteCherryPickRequest(@PathVariable("projectName") String
	 * projectName,
	 * 
	 * @PathVariable("creatorName") String creatorName,@PathVariable("id")
	 * String id){ Weaver currentWeaver = weaverService.getCurrentWeaver();
	 * CherryPickRequest cherryPickRequest = cherryPickRequestService.get(id);
	 * cherryPickRequestService.delete(cherryPickRequest, currentWeaver); return
	 * "redirect:/project/"+creatorName+"/"+projectName+"/cherry-pick"; }
	 * 
	 * /* //코드에 답변달기
	 * 
	 * @RequestMapping(value =
	 * "/{creatorName}/{projectName}/commitlog-viewer/commit:{commit}/add-repost",
	 * method = RequestMethod.POST) public String
	 * addRepost(@PathVariable("projectName") String projectName,
	 * 
	 * @PathVariable("creatorName") String creatorName,
	 * 
	 * @PathVariable("commit") String commit,HttpServletRequest request) throws
	 * UnsupportedEncodingException { final MultipartHttpServletRequest
	 * multiRequest = (MultipartHttpServletRequest) request;
	 * 
	 * final Map<String, MultipartFile> files = multiRequest.getFileMap();
	 * 
	 * String content = request.getParameter("content"); Weaver weaver =
	 * weaverService.getCurrentWeaver(); Project project =
	 * projectService.get(creatorName+"/"+projectName); GitCommitLog
	 * gitCommitLog = gitService.getGitCommitLog(creatorName, projectName,
	 * commit); if(project == null || weaver == null || gitCommitLog == null ||
	 * content.equals("")) // 권한 검사,로그인 검사, 글 존재 여부 검사, 내용 존재 여부 검사. return
	 * "redirect:/project/"+project.getName()+"/commitlog-viewer/commit:"+
	 * commit; Weaver commiter =
	 * weaverService.get(gitCommitLog.getCommiterEmail());
	 * 
	 * ArrayList<Data> datas = new ArrayList<Data>(); for (MultipartFile file :
	 * files.values()) { if(!file.isEmpty()) datas.add(new
	 * Data(dataService.getObjectID(file.getOriginalFilename(),
	 * weaver),file,weaver.getId())); }
	 * 
	 * RePost rePost = new
	 * RePost(project.getName()+"/"+gitCommitLog.getCommitLogID(), commiter,
	 * weaver, WebUtil.specialSignDecoder(URLDecoder.decode(content))),
	 * project.getTags(), 5); rePostService.add(rePost,datas);
	 * 
	 * return
	 * "redirect:/project/"+project.getName()+"/commitlog-viewer/commit:"+
	 * commit; }
	 * 
	 * @RequestMapping(value =
	 * "/{creatorName}/{projectName}/commitlog-viewer/commit:{commit}/{rePostID}/add-reply",
	 * method = RequestMethod.POST) public String
	 * addReply(@PathVariable("projectName") String projectName,
	 * 
	 * @PathVariable("creatorName") String creatorName,
	 * 
	 * @PathVariable("commit") String commit,@PathVariable("rePostID") int
	 * rePostID,HttpServletRequest request) throws UnsupportedEncodingException
	 * { String content = request.getParameter("content"); RePost rePost =
	 * rePostService.get(rePostID); Project project =
	 * projectService.get(creatorName+"/"+projectName); GitCommitLog
	 * gitCommitLog = gitService.getGitCommitLog(creatorName, projectName,
	 * commit); Weaver weaver = weaverService.getCurrentWeaver();
	 * 
	 * if(project == null || weaver == null || gitCommitLog == null ||
	 * content.equals("")) // 권한 검사,로그인 검사, 답변 존재 여부 검사, 글 존재 여부 검사, 내용 존재 여부
	 * 검사. return
	 * "redirect:/project/"+project.getName()+"/commitlog-viewer/commit:"+
	 * commit;
	 * 
	 ...
	 * 
	 * @RequestMapping(value=
	 * "/{creatorName}/{projectName}/commitlog-viewer/commit:{commit}/{rePostID}/{number}/delete")
	 * public String deleteReply(@PathVariable("projectName") String
	 * projectName,
	 * 
	 * @PathVariable("creatorName") String creatorName,
	 * 
	 ...
	 * 
	 * @PathVariable("number") int number, HttpServletRequest request) { Project
	 * project = projectService.get(creatorName+"/"+projectName); RePost rePost
	 * = rePostService.get(rePostID); Weaver weaver =
	 * weaverService.getCurrentWeaver(); GitCommitLog gitCommitLog =
	 * gitService.getGitCommitLog(creatorName, projectName, commit); if( project
	 * == null || weaver == null || gitCommitLog == null ||
	 * !rePost.removeReply(weaver, number)) // 권한 검사,로그인 검사, 답변 존재 여부 검사, 글 존재
	 * 여부 검사, 내용 존재 여부 검사. return
	 * "redirect:/project/"+project.getName()+"/commitlog-viewer/commit:"+
	 * commit;
	 * 
	 * rePostService.update(rePost,null);
	 ...
	 * 
	 * @RequestMapping(
	 * "/{creatorName}/{projectName}/commitlog-viewer/commit:{commit}/{rePostID}/delete")
	 * public String deleteRePost(Model model, @PathVariable("projectName")
	 * String projectName,
	 * 
	 * @PathVariable("creatorName") String creatorName,
	 * 
	 * @PathVariable("commit") String commit, @PathVariable("rePostID") int
	 * rePostID) { Project project =
	 * projectService.get(creatorName+"/"+projectName); RePost rePost =
	 * rePostService.get(rePostID); Weaver weaver =
	 * weaverService.getCurrentWeaver(); rePostService.delete(rePost,weaver);
	 * return
	 * "redirect:/project/"+project.getName()+"/commitlog-viewer/commit:"+
	 * commit; }
	 */
